/*
 * kdb3database.h - <application name>
...
#ifndef KDB3DATABASE_H
#define KDB3DATABASE_H

#include <QObject>
#include <QThread>
#include <QMap>
#include "keepassx.h"   // BUILTIN_ICONS, extern EntryIcons, extern config
#include "database.h"
...
public:
    ...
    virtual QPixmap& icon(quint32 index);
    virtual int numIcons();
    virtual void addIcon(const QPixmap& icon);
    virtual void removeIcon(int index);
    virtual void replaceIcon(quint32 index,const QPixmap& icon);
    virtual quint32 builtinIcons(){return BUILTIN_ICONS;}
